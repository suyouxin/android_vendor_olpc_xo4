...
//#define CAM_ALOG LOGE
#define CAM_LOG ALOGI
...
static void v4l_cam_streaming_stop(struct v4l_cam *cam, struct buf_set *set)
{
    ...
    CAM_LOG("%s(): TRACE LINE: %d", __func__, __LINE__);
...
/* static int v4l_balance_set(struct v4l_cam * cam)  */
/* { */
    /* struct v4l2_control ctl; */
    /* ctl.id = V4L2_CID_AUTO_WHITE_BALANCE; */
    /* ctl.value = 1; */
    /* return ioctl(cam->fd, VIDIOC_S_CTRL, &ctl) < 0 ? -errno : 0; */
/* } */
...
static void v4l_cam_destroy_buffers(struct v4l_cam *cam)
{
    ...
    /* req.count = 0; */
    /* req.type = cam->buf_picture.fmt.type; */
    /* req.memory = cam->buf_picture.memory; */
    /* ioctl(cam->fd, VIDIOC_REQBUFS, &req); */

    /* req.count = 0; */
	/* req.type = cam->buf_record.fmt.type; */
...
static int v4l_cam_buf_create(struct v4l_cam *cam, struct buf_set *set, int n)
{
	...
	CAM_LOG("CREATE_BUFS memory type: %d count: %d",
		create.memory,
		create.count);
	 ...
	 */
	unsigned int i;
	int ret = ioctl(cam->fd, VIDIOC_REQBUFS, &req);
...
static int v4l_cam_umem_by_addr(const struct buf_set *set, const uint8_t *ptr)
{
	...
	ALOGE("USERPTR: Buffer %p not found!\n", ptr);
...
static int v4l_cam_ubuf_init(struct v4l_cam *cam, struct buf_set *set)
{
	...
	set->buffers[0].cam_mem = cam->cb.get_mem(cam->ion_fd, sizeimage, set->num_buffers,
						  cam->cb.arg);
...
static int v4l_cam_mbuf_init(struct v4l_cam *cam, struct buf_set *set)
{
	...
	for (i = 0; i < set->num_buffers; i++) {
        ...
        /* set->buffers[i].cam_mem = cam->cb.get_mem(cam->ion_fd, buf.length, */
        set->buffers[i].cam_mem = cam->cb.get_mem(-1, buf.length,
							  1, cam->cb.arg);
...
static int v4l_cam_configure(struct v4l_cam *cam)
{
    ...
    /* int ret = v4l_balance_set(cam); */
	/* CAM_LOG("%s(): S_CTRL() = %d", __func__, ret); */
    ...
    cam->buf_picture.num_buffers = req.count;
    cam->buf_picture.index = 0;

    req.count = NUM_BUF_RECORD;
    req.type = cam->buf_record.fmt.type;
    req.memory = cam->buf_record.memory;
    ret = ioctl(cam->fd, VIDIOC_REQBUFS, &req);
    if (ret < 0)
    ...
    cam->buf_record.num_buffers = req.count;
    cam->buf_record.index = 0;
...
static int v4l_cam_buf_queue(struct v4l_cam *cam, struct buf_set *set, unsigned int n)
{
	...
	case V4L2_MEMORY_USERPTR:
        ...
        CAM_LOG("buf.m.userptr %p len = %d", buf.m.userptr, buf.length);
		break;
	default:
		return -ENOSYS;
	case V4L2_MEMORY_MMAP:
...
static void v4l_cam_enable_msg_type(struct camera_device *dev, int32_t msg_type)
{
	...
	CAM_LOG("%s(0x%x), cam->msg_mask:0x%x, cam->msg:0x%x", __func__, msg_type, 
            cam->msg_mask, cam->msg);
}
...
#define v4l_cam_try_or_bug(fd, fmt) do {				\
	if (ioctl((fd), VIDIOC_TRY_FMT, (fmt)) < 0) {			\
		ALOGE("%s(): buggy camera driver! VIDIOC_TRY_FMT should never fail!", \
			__func__);					\
		return -errno;						\
	}								\
} while (0)
...
static int v4l_cam_default_fmts(struct v4l_cam *cam)
{
	...
	while (!v4l_cam_try_buftype(cam, VIDIOC_ENUM_FMT, &fmtdesc)) {
		...
		if (fmtdesc.pixelformat == V4L2_PIX_FMT_JPEG)
			...
			cam->picture_compressed_4cc = V4L2_PIX_FMT_YUYV;
	...
	if (cam->picture_compressed_4cc) {
		...
		if (cam->picture_compressed_4cc == V4L2_PIX_FMT_JPEG)
			...
			v4l_cam_make_fmt(&fmt, V4L2_PIX_FMT_YUYV, V4L2_COLORSPACE_JPEG,
					 -1, 0, 0, return -EINVAL);
		}
				  ...
				  const char *parms)
{
    ...
    if (ret == -EBUSY) {
        CAM_LOG("%s() configure failed because of EBUSY, ignore it", __func__);
        return 0;
    }
	if (ret < 0)
		return ret;
...
static int v4l_cam_picture(struct v4l_cam *cam)
{
	...
	if (picture_err < 0) {
		cam->picture = false;
		ALOGE("%s(): picture capture failed: %d", __func__, picture_err);
		goto preview;
	}
...
preview:
	...
	picture_err = v4l_cam_format_set(cam, &cam->buf_preview.fmt);
	if (picture_err < 0) {
        CAM_LOG("%s(): TRACE LINE: %d", __func__, __LINE__);
	}
	...
	if (cam->picture) {
		cam->picture = false;
        CAM_LOG("%s(): TRACE LINE: %d, idx:%d, pic index:%d", __func__, __LINE__,
                idx, idx - cam->buf_picture.index);
	...
	/* if (ret < 0) */
		/* ALOGV("%s(): prepare thread failed: %d, VIDIOC_QBUF will be slower.", */
			 /* __func__, ret); */
...
static int v4l_cam_frame_done(struct v4l_cam *cam, struct buf_set *set, int idx)
{
	...
	if (cam->record && (cam->msg & CAMERA_MSG_VIDEO_FRAME)) {
		...
		ALOGV("%s(%d) @ %lld", __func__, idx, timestamp);
	...
	if (ops) {
		...
		if (ret != NO_ERROR || !buffer) {
			ALOGE("%s(): Unable to dequeue preview window buffer @ %p: %d",
			     __func__, buffer, ret);
			return ret;
		}
		...
		if (ret != NO_ERROR) {
			ALOGE("%s(): Unable to lock preview window buffer: %d",
			     __func__, ret);
			goto fail;
		}
		...
		if (!img) {
			ALOGE("%s(): Unable to lock preview window output buffer",
			     __func__);
			ret = -ENOBUFS;
			goto fail;
		}
		...
		if (cam->msg & CAMERA_MSG_VIDEO_FRAME) {
			...
			ALOGV("%s(%d) %s", __func__, idx, locked ? "wins" : "loses");
			if (locked)
		...
		if (cam->record)
			ALOGV("%s(): enqueue() = %d", __func__, ret);

		camera_preview_unlock(buffer);
		if (ret != NO_ERROR) {
			ALOGE("%s(): Unable to convert preview window buffer: %d",
			     __func__, ret);
			goto fail;
		}
        ALOGE("trace line: %s %d\n", __FILE__, __LINE__);
	}

	ALOGV("%s(): queue(%d)", __func__, idx);
	/* Always queue next? */
...
static void *v4l_cam_thread_preview(void *arg)
{
	...
	while (true) {
		...
		if (!cam->preview) {
			...
			pthread_mutex_lock(&cam->thread_mutex);
			if (ret < 0) {
				ALOGE("Streaming: STREAMON failed: %d, stopping\n", ret);
		...
		ALOGV("%s(): hunger!", __func__);

		idx = v4l_cam_buf_dequeue(cam, set);
		if (idx < 0) {
			ALOGE("Streaming: DQBUF failed: %d, stopping\n", idx);
		...
		ALOGV("%s(%d, ops %p): yumm!", __func__, idx, cam->preview_ops);

		ret = v4l_cam_frame_done(cam, set, idx - set->index);
		if (ret < 0) {
			ALOGE("Streaming: QBUF failed: %d, stopping\n", ret);
		...
		if (cam->record_start && !cam->record) {
			...
			if (!ret)
				...
				ALOGE("%s(): failed to start recording: %d", __func__, ret);
		}
		...
		if (cam->record_stop) {
			...
			if (ret < 0) {
				ALOGE("Streaming: preview failed: %d, stopping\n", ret);
			...
			struct hw_device_t **device)
{
	...
	v4l_cam_dev.ion_fd = open("/dev/ion", O_RDWR);
	if (v4l_cam_dev.ion_fd < 0) {
		ALOGE("open(ion dev): errno = %d", errno);
		return -errno;
	}

	v4l_cam_dev.fd = open(v4l_cam_node[cam_id], O_RDWR);
	if (v4l_cam_dev.fd < 0) {
		ALOGE("open(%s): errno = %d", v4l_cam_node[cam_id], errno);
		return -errno;
	}

	ret = ioctl(v4l_cam_dev.fd, VIDIOC_QUERYCAP, &cap);
	if (ret < 0 || !(cap.capabilities & V4L2_CAP_STREAMING)) {
		ALOGE("QUERYCAP(%s): errno = %d, caps=%x", v4l_cam_node[cam_id], errno, cap.capabilities);
		goto ecap;
	}
	...
	if (cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)
		...
		ALOGE("No CAPTURE cap in %x", cap.capabilities);
		goto ecap;
	}
	 ...
	 */
	v4l_cam_dev.buf_preview.memory = V4L2_MEMORY_USERPTR;
	v4l_cam_dev.buf_picture.memory = V4L2_MEMORY_USERPTR;
	v4l_cam_dev.buf_record.memory = V4L2_MEMORY_USERPTR;
	...
	if (ret < 0) {
		ALOGE("pthread_create() = %d, errno = %d", ret, errno);
		goto ecrtpvu;
	}
	...
	*device = &v4l_cam_dev.acam.common;
	...
	ALOGI("%s: opened camera #%s (%p), type %u", __func__, id, *device, v4l_cam_dev.buf_preview.fmt.type);
